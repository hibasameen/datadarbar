...
GEO_OUT = os.path.join(HERE, "..", "..", "app", "data", "tehsils_geo.js")
POV = os.path.join(HERE, "..", "..", "app", "data", "poverty_data.js")
...
def main():
    ...
    for dd_id, rs in groups.items():
        ...
        meta = teh.get(dd_id, {})
        rec = {"m": tot, "np": len(rs),
               "name": meta.get("name", ""), "dk": meta.get("dk", ""),
               "prov": meta.get("prov", "")}
        if approx[dd_id]:
            rec["apx"] = 1
        if tot == 0:
    ...
    for k, v in teh.items():
        payload.setdefault(k, {"cov": 0, "m": 0, "np": 0, "name": v.get("name", ""),
                               "dk": v.get("dk", ""), "prov": v.get("prov", "")})
    ...
    with open(OUT, "w") as f:
        ...
        f.write(";\n")
    # ADM3 geometry, lifted out of the poverty payload. map.html fetches this
    # only when a tehsil group is selected — it is 600 KB and most visits never
    # leave the district geography.
    src = open(POV).read()
    a = src.index("window.DD_POV_GEO_T=") + len("window.DD_POV_GEO_T=")
    b = src.index("\n", a)
    with open(GEO_OUT, "w") as f:
        f.write("/* Data Darbar - tehsil (ADM3) geometry, generated by "
                "etl/mouza2020; do not edit by hand */\n")
        f.write("window.DD_GEO_T=" + src[a:b].rstrip(";") + ";\n")
    print(f"geometry: {len(open(GEO_OUT).read()):,} bytes -> {GEO_OUT}")
